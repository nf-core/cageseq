...
**nf-core/cageseq** is a Nextflow pipeline to process CAGE sequencing data from raw reads to the identification of consensus clusters and enhancers.
...
```bash
nextflow run cageseq/main.nf -params-file params.yaml -profile docker
```
...
```bash
nextflow run cageseq/main.nf \
    --bsgenome BSgenome.Scerevisiae.UCSC.sacCer3 \
    --gtf cageseq/assets/sacCer3_genome/sacCer3.ensGene.gtf \
    --index cageseq/assets/sacCer3_genome/sacCer3_star_index/ \
    --input cageseq/assets/samplesheet_sacer_pe_template.csv \
    -profile singularity \
...
nextflow run cageseq/main.nf \
    ...
    --gtf cageseq/assets/sacCer3_genome/sacCer3.ensGene.gtf \
    --fasta cageseq/assets/sacCer3_genome/sacCer3.fa \
    --gtf cageseq/assets/sacCer3_genome/sacCer3.ensGene.gtf \
    --infolder cageseq/assets/sacCer_fastq/pe_per_sample/ \
    -profile singularity
```
...
```bash
cageseq/assets/sacCer_fastq/pe_per_sample/
├── S10
...
nextflow run cageseq/main.nf \
    ...
    --gtf cageseq/assets/sacCer3_genome/sacCer3.ensGene.gtf \
    --bowtie2 \
    --index cageseq/assets/sacCer3_genome/sacCer3_bowtie2_index \
    --dedup \
    --dist 100 \
    --input cageseq/assets/samplesheet_sacer_pe_template.csv \
    -profile singularity
...
```
nextflow run cageseq/main.nf \
    -params-file cageseq/testdata/params_yeast_borlin_test.yaml \
    -profile singularity \
    -w work_se_test
```
...
```
nextflow run cageseq/main.nf \
    -params-file cageseq/testdata/params_danio_nepal_test.yaml \
    -profile singularity \
    -w work_pe_test
```
